...
```
MY OSM/
├── README.fr.md     ← ce fichier : la carte du projet
├── README.md        ← la même, en anglais
...
│
├── docs/            ← architecture de la couche transport, audit
│
├── livrables/       ← CE QU'ON INSTALLE (hors du dépôt)
│   ├── MY-OSM-debug.apk      dernier APK de travail (débogable)
│   ├── MY-OSM.apk            dernier APK à partager (release)
│   └── anciens/              APK précédents, datés
│
├── saves/           ← SAUVEGARDES DATÉES, une par grosse modification (hors du dépôt)
...
Aucune clé n'est nécessaire : l'application fonctionne sans, et les transports
passent alors partout par Transitous. Des clés gratuites et facultatives
(Île-de-France Mobilités PRIM, TomTom, Mapillary, Météo-France) se saisissent dans
l'application (Menu › API), ou dans `app/.env.local` pour une construction locale
(voir `app/.env.example`). Elles ne sont jamais versionnées.
...
dans l'application (Menu › Sources et licences).
